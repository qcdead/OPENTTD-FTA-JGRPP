		...
		switch (widget) {
			...
			case WID_AIC_INCREASE_NUMBER: {
				...
				if (widget == WID_AIC_DECREASE_NUMBER) {
					new_value = std::max(0, static_cast<int>(GetGameSettings().difficulty.max_no_competitors - 1));
				} else {
					new_value = std::min(MAX_COMPANIES - 1, static_cast<int>(GetGameSettings().difficulty.max_no_competitors + 1));
				}
