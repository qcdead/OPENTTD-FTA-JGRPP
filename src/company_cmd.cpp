...
void ResetCompanyLivery(Company *c)
{
	for (LiveryScheme scheme = LS_BEGIN; scheme < LS_END; scheme++) {
		...
		c->livery[scheme].colour1 = (Colours)c->colour;
		c->livery[scheme].colour2 = (Colours)c->colour;
	}

	for (Group *g : Group::Iterate()) {
		if (g->owner == c->index) {
			g->livery.in_use  = 0;
			g->livery.colour1 = (Colours)c->colour;
			g->livery.colour2 = (Colours)c->colour;
		}
	}
}
