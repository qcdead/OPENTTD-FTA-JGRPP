...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SLV_104)) {
		...
		/* More companies ... */
		for (Company *c : Company::Iterate()) {
			if (c->bankrupt_asked.base() == 0x00000000000000FF) c->bankrupt_asked.Set();
		}

		for (Engine *e : Engine::Iterate()) {
			if (e->company_avail.base() == 0x00000000000000FF) e->company_avail.Set();
		}

		for (Town *t : Town::Iterate()) {
			if (t->have_ratings.base() == 0x00000000000000FF) t->have_ratings.Set();
