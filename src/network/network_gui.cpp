...
struct NetworkStartServerWindow : public Window {
	int16_t widget_id;          ///< The widget that has the pop-up input menu
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_NSS_COMPANIES_TXT:  // Click on number of companies
				...
				ShowQueryString(GetString(STR_JUST_INT, _settings_client.network.max_companies), STR_NETWORK_START_SERVER_NUMBER_OF_COMPANIES,  4, this, CS_NUMERAL, QSF_NONE);
				break;
